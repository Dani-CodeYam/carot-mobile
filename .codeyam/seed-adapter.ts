...
export function main() {
  ...
  // Output the localStorage map as JSON to stdout, FLAT — one top-level key
  // per localStorage entry.
  //
  // NOT `{ localStorage: {...} }`, which is what the shipped v6 template emits:
  // the binary parses this stdout as a flat key→value map (see
  // parse_local_storage_from_stdout in crates/control-api/src/seed.rs), so the
  // wrapper lands as a SINGLE localStorage entry literally named
  // "localStorage" holding the whole map as a string. Every seeded scenario
  // then renders its empty state, silently. Verify with
  // `codeyam-editor editor capture-state '{"slug":"<slug>"}'` — the injected
  // keys should be the app's own, never a lone "localStorage".
  const out = transformSeed(seed);
  console.log(JSON.stringify(out.localStorage, null, 2));
